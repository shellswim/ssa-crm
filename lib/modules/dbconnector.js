...
async function _processSubQueries(db, results, sub) {
    ...
    for (const field in sub) {
        ...
        for (const subResult of subResults) {
            const results = lookup.get(subResult['__dmxForeign']);

            for (const result of results) {
                result[field].push(subResult);
            }

            delete subResult['__dmxPrimary'];
            delete subResult['__dmxForeign'];
        }
    }
